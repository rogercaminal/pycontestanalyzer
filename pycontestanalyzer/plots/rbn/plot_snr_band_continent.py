...
from pandas import Grouper, DataFrame, date_range, merge
...
class PlotSnrBandContinent(PlotReverseBeaconBase):
    ...
    def __init__(
        ...
        """
        super().__init__(
            contest=contest, mode=mode, years=[year]
        )
    ...
    def _clean_dataset(self):
        ...
        df_grp = (
            _df
            .query(f"dx.isin({self.callsigns})")
            .query(f"de_cont.isin({self.rx_continents})")
            .assign(freq=lambda x: np.round(x["freq"]).astype(int))
        )

        # Make sure that everyone is spotted by the same list of RX through the whole contest.
        if self.closed_list_spotters:
            logger.info("Define closed list of RX spotters")
            df_temp = (
                df_grp
                .groupby(['dx'], as_index=False)
                .agg(unique_callsign=("callsign", "unique"))
            )
            common_callsigns = set()
            for i, call in enumerate(self.callsigns):
                if i == 0:
                    common_callsigns = set(df_temp.query(f"dx == '{call}'")["unique_callsign"].values[0].tolist())
                else:
                    common_callsigns &= set(df_temp.query(f"dx == '{call}'")["unique_callsign"].values[0].tolist())
            df_grp = df_grp.query(f"callsign.isin({list(common_callsigns)})")

        # Consider only those RX spotters that have announced a DX per band >= n times in an hour
        if self.min_rx_spots_per_hour:
            logger.info(f"Filtering out RX stations with less than {self.min_rx_spots_per_hour} spots per hour and band")
            df_grp = (
                df_grp
                .groupby([Grouper(key="datetime", freq=f"60Min"), 'dx', 'callsign', 'band'], as_index=False)
                .apply(
                    lambda x: x.assign(counts_rx=x["callsign"].count())
                )
        ...
        df_grp = (
            df_grp
            .groupby([Grouper(key="datetime", freq=f"{self.time_bin_size}Min"), "dx", "callsign", "freq", "band", "de_cont"], as_index=False)
            .agg(
                db=("db", "mean"),
            )
        )
        ...
        df_list = [
            DataFrame(date_range(df_grp["datetime"].min(), df_grp["datetime"].max(), freq="10Min"), columns=["datetime"]),
        ...
        df_grp = (
            df_grp
            .groupby(["dx", "band", "de_cont", "callsign"], as_index=False)
            .apply(
                lambda x: x.assign(
                    db=np.where(x["db"].isnull(), (x["db"].shift(1) / 2. + x["db"].shift(-1) / 2.), x["db"])
                )
            )
        )
        ...
        df_grp = (
            ...
            .apply(
                lambda x: x.assign(
                    ...
                    db_exp_rev=x.iloc[::-1]["db"].transform(lambda y: y.ewm(alpha=0.3).mean()).iloc[::-1]
                )
            )
            .assign(db=lambda x: (x["db_exp"] + x["db_exp_rev"])/2.)
        ...
        df_grp = (
            df_grp
            .groupby(["datetime", "dx", "band", "de_cont"], as_index=False)
            .agg(db=("db", "median"))
        )
        ...
        df_grp = (
            df_grp
            .groupby(["dx", "band", "de_cont"], as_index=False)
            .apply(
                lambda x: x.assign(
                    db=x["db"].transform(lambda x: x.rolling(int(self.smoothing * 60 / self.time_bin_size), min_periods=1, center=True).median())
                )
            )
        )
        return df_grp
