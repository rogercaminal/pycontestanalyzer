...
class PlotRatioQSOsMin(PlotBase):
    ...
    def do_plot(self, contest, doSave, options=""):
        ...
        if options == "all":
            qsos_day1 = (
                contest.log[
                    (contest.log["isdupe"] == False)
                    & (contest.log["date"] == contest.log["date"].iloc[0])
                ]["datetime"]
                .value_counts()
                .fillna(0)
            )
            qsos_day2 = (
                contest.log[
                    (contest.log["isdupe"] == False)
                    & (contest.log["date"] == contest.log["date"].iloc[-1])
                ]["datetime"]
                .value_counts()
                .fillna(0)
            )
        elif options == "running":
            qsos_day1 = contest.log[
                (contest.log["isdupe"] == False)
                & (contest.log["date"] == contest.log["date"].iloc[0])
                & (contest.log["station_type"] == "running")
            ]["datetime"].value_counts()
            qsos_day2 = contest.log[
                (contest.log["isdupe"] == False)
                & (contest.log["date"] == contest.log["date"].iloc[-1])
                & (contest.log["station_type"] == "running")
            ]["datetime"].value_counts()
        ...
        data = [
            go.Histogram(
                x=qsos_day2,
                name="Day 2",
                xbins=dict(start=1, end=15, size=1),
                autobinx=False,
