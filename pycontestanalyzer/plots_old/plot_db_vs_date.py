...
class PlotDbVsDate(PlotBase):
    ...
    def do_plot(self, contest, doSave, options=""):
        ...
        # --- Extra conditions
        extra_conditions = contest.rbspots["db"] > 0.0
        avg = "15min"
        band = None
        ...
        contest.rbspots["date_round%s" % avg] = contest.rbspots[
            "date"
        ].dt.round(avg)
        ...
        y[10] = (
            contest.rbspots[
                (contest.rbspots["band"] == "10m") & extra_conditions
            ]
            .groupby("date_round%s" % avg)["db"]
            .mean()
        )
        y[15] = (
            contest.rbspots[
                (contest.rbspots["band"] == "15m") & extra_conditions
            ]
            .groupby("date_round%s" % avg)["db"]
            .mean()
        )
        y[20] = (
            contest.rbspots[
                (contest.rbspots["band"] == "20m") & extra_conditions
            ]
            .groupby("date_round%s" % avg)["db"]
            .mean()
        )
        y[40] = (
            contest.rbspots[
                (contest.rbspots["band"] == "40m") & extra_conditions
            ]
            .groupby("date_round%s" % avg)["db"]
            .mean()
        )
        y[80] = (
            contest.rbspots[
                (contest.rbspots["band"] == "80m") & extra_conditions
            ]
            .groupby("date_round%s" % avg)["db"]
            .mean()
        )
        y[160] = (
            contest.rbspots[
                (contest.rbspots["band"] == "160m") & extra_conditions
            ]
            .groupby("date_round%s" % avg)["db"]
            .mean()
        )
            ...
            if band is None:
                data.append(
                    go.Scatter(
                        x=x[b],
                        y=y[b],
                        line=dict(color=(colors[b]), width=1),
                        hoverinfo="x+y",
                        mode="lines",
                        name="%dm" % b,
                    )
