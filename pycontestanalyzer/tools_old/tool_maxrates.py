...
class ToolMaxRates(ToolBase):
    ...
    def apply_to_all(self, contest):
        ...
        date_range = daterange(
            pd.to_datetime("%s 00:00:00" % contest.log["date"].iloc[0]),
            pd.to_datetime("%s 23:59:00" % contest.log["date"].iloc[-1]),
        )
        ...
        for mins in steps:
            ...
            for d in range(0, len(date_range) - mins):
                qsos = contest.log[
                    (contest.log["datetime"] >= date_range[d])
                    & (contest.log["datetime"] < date_range[d + mins])
                ]["call"].count()
                if qsos > max_qsos:
                    max_qsos = qsos
        ...
        for mins in steps:
            contest.log["maxRate_%dmin" % mins] = np.where(
                (
                    (
                        contest.log["datetime"]
                        >= contest.max_rates["%dmin" % mins][1][0]
                    )
                    & (
                        contest.log["datetime"]
                        < contest.max_rates["%dmin" % mins][1][1]
                    )
                ),
                1,
                0,
        ...
        for i, date in enumerate(listDates):
            for h in range(24):
                ...
                for m in range(60):
                    ...
                    counts = contest.log[
                        (contest.log["date"] == date)
                        & (contest.log["time"] == time)
                    ]["call"].count()
