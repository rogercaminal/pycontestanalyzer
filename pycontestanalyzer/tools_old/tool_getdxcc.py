...
class ToolGetDXCC(ToolBase):
    ...
    def apply_to_row(self, row):
        ...
        try:
            ...
            row["locator"] = latlong_to_locator(
                dxcc["latitude"], dxcc["longitude"]
            )
        except:
            row["dxcc"] = np.NaN
            row["zonecq"] = np.NaN
    ...
    def apply_to_all(self, contest):
        ...
        contest.log["zoneitu"] = (
            contest.log["zoneitu"].fillna(-1).astype("int")
        )
        ...
        if len(dxcc.keys()) > 0:
            ...
            contest.log["mylocator"] = latlong_to_locator(
                float(dxcc["latitude"]), float(dxcc["longitude"])
            )
            contest.log["heading"] = contest.log.apply(
                lambda row: calculate_heading(row["mylocator"], row["locator"])
                if isinstance(row["locator"], str)
                else np.NaN,
                axis=1,
            )
            contest.log["heading_long"] = contest.log.apply(
                lambda row: calculate_heading_longpath(
                    row["mylocator"], row["locator"]
                )
                if isinstance(row["locator"], str)
                else np.NaN,
                axis=1,
            )
            contest.log["distance"] = contest.log.apply(
            ...
            contest.log["distance_long"] = contest.log.apply(
                lambda row: calculate_distance_longpath(
                    row["mylocator"], row["locator"]
                )
                if isinstance(row["locator"], str)
                else np.NaN,
