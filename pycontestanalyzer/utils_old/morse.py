...
class MorseConverter(object):
    def __init__(self, wpm):
        self.wpm = wpm
        self.bps = wpm * 50.0 / 60  # --- PARIS has 50 bits
        self.initial_string = ""
        self.morse_string = ""
        self.bits_string = ""
        self.nbits = 0
    ...
    def setString(self, initial_string):
        ...
        for b in self.bits_string:
            self.nbits += 1
            self.time_seconds += 1.0 / self.bps
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest="doVerbose",
        default=False,
        help="Set the program in verbose mode",
    )
    parser.add_option(
        "--wpm", action="store", type="string", default=30, help="Speed in WPM"
    )
    parser.add_option(
        "--text",
        action="store",
