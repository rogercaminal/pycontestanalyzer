...
from pycontestanalyzer.cli.main import app
...
def test_app_with_help_exits_correctly(runner: CliRunner):
    ...
    assert "Usage: pycontestanalyzer [OPTIONS] COMMAND [ARGS]" in result.stdout
...
def test_app_command_with_help_exit_correclty(runner: CliRunner, command: str):
    ...
    assert result.exit_code == 0
    assert (
        f"Usage: pycontestanalyzer {command} [OPTIONS] COMMAND [ARGS]" in result.stdout
    )